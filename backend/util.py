...
def haversine(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    ...
    a = func.sin(dlat / 2) * func.sin(dlat / 2) + func.cos(
        func.radians(lat1)
    ) * func.cos(func.radians(lat2)) * func.sin(dlon / 2) * func.sin(dlon / 2)
    ...
    def get_closest_stores(
        ...
        try:
            ...
            subq = session.query(
                ...
                func.row_number()
                .over(
                    partition_by=StoreModel.Chain,
                    order_by=haversine(StoreModel.Lat, StoreModel.Long, lat, lon),
                )
                .label("row_num"),
            ).subquery()
            ...
            logging.error(
                f"An unexpected error occurred while finding closest stores. Error: {e}"
            )
            return []
...
def get_youngest_search(term: str, zip_code: str) -> SearchDomain | None:
    ...
    try:
        result = (
            session.query(SearchModel)
            .filter(SearchModel.Term == term, SearchModel.ZipCode == zip_code)
            .order_by(desc(SearchModel.LastModified))
            .first()
        )
...
def fetch_db_prices_search(search: SearchDomain) -> Optional[List[dict]]:
    ...
    try:
        ...
        results = (
            session.query(StorePrice).filter(StorePrice.SearchID == search.id).all()
        )
...
def save_search_data(
    search_domain: SearchDomain, data: List[dict]
) -> SearchDomain | None:
    """
    ...
    try:
        ...
        # if it's not existing we create a new one
            ...
            search_entry = SearchModel(
                Term=search_domain.term, ZipCode=search_domain.zip_code
            )
        ...
        for item in data:
            ...
            if not product:
                product = Product(Name=product_data["name"], UPC=product_data["UPC"])
                session.add(product)
...
async def get_prices(term: str, zip_code: str, lat: Decimal, lon: Decimal) -> List[dict]:
    ...
    try:
        ...
        results = await fetch_best_prices(term=term, stores=stores)
        ...
        if not saved:
            logging.error(
                f"Failed to save search data for term: {term}, zip_code: {zip_code}"
            )
            return []
        ...
        logging.error(
            f"Database error occurred while fetching prices for term: {term}, zip_code: {zip_code}"
        )
        return []

    except requests.exceptions.RequestException:
        logging.error(
            f"Connection error while fetching prices for term: {term}, zip_code: {zip_code}"
        )
        return []

    except Exception as e:
        logging.error(
            f"An unexpected error occurred while fetching prices for term: {term}, zip_code: {zip_code}. Error: {e}"
        )
        return []
...
def update_stores(stores: List[dict]) -> None:
    ...
    try:
        for store in stores:
            ...
            store_entry = (
                session.query(Store)
                .filter_by(StoreAPIRef=store["api_reference"], Chain=store["chain"])
                .first()
            )
            if not store_entry:
                store_entry = Store(
